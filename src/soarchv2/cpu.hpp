...
#ifndef EZCPU_CPUV2_CPU_HPP
#define EZCPU_CPUV2_CPU_HPP
...
namespace SOCPU::Sim::CPUv2{
	...
	struct CPU:Circuit{
		...
		NOT<1> rw{name+"[RW]"},rr{name+"[RR]"},clk_{name+"[CLK#]"};
		...
		explicit CPU(std::string name=""): Circuit(std::move(name)), regs{
			...
			add_comps(clk_,sreg,CU,rw,rr,mw_or,mi,mo,ALU);
			...
			bus.CLK >> sreg.clk >> regs[0].clk >> mw_or.A;//>> clk_.A;
			//clk_.Y >> mw_or.A;
		...
		void init(){
			...
			CU.fn=[](ARG::val_t i){
				...
				//return InstrSet::gen(Config{i}).ctl.val();
			};
			/*for(auto&& [i,cu]:zip(iota(ARG::val_t{0}),CU)){
				cu=InstrSet::gen(Config{i}).ctl.val();
			}*/
		}
